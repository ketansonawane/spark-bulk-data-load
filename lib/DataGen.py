...
def gen_accounts_df(spark, load_date, num_records):
    print("Inside gen_accounts_df() method")
    ...
    data_list = [(load_date, choices([0,1],cum_weights=[10, 90], k=1 )[0],
                 ...
                 for i in tqdm(range(1, num_records))]
...
def gen_account_party(spark, load_date, num_records):
    print("Inside gen_account_party() method")
    ...
    data_list_s = [(load_date, account_id_offset + fake.pyint(1, num_records),
                    ...
                    fake.date_time_between(start_date='-5y', end_date='-3y'))
                   for i in tqdm(range(1, int(num_records / 3)))]
...
def gen_party_address(spark, load_date, num_records):
    print("Inside gen_party_address() method")
...
def create_data_files(spark, load_date, num_records):
    ...
    #     .write.format("parquet").option("compression", "snappy").mode("overwrite").save("test_data/accounts")
    #
    # party_df = gen_account_party(spark, load_date, num_records)
    # party_df.coalesce(1) \
    #     .write.format("parquet").option("compression", "snappy").mode("overwrite").save("test_data/parties")

    address_df = gen_party_address(spark, load_date, num_records)
    address_df.coalesce(1) \
        .write.format("parquet").option("compression", "snappy").mode("overwrite").save("test_data/party_address")

spark = Utils.get_spark_session("local")
create_data_files(spark, '2023-09-07', 2000000)
