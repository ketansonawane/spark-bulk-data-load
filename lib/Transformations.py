from pyspark.sql.functions import struct, lit, col, isnull, when, array, filter, collect_list, expr, date_format, \
    current_timestamp
...
def join_party_and_address(p_df, a_df):
...
def join_contract_and_party(c_df, p_df):
    return c_df.join(p_df, 'account_id', 'left')

def apply_headers(spark, df):
    ...
    event_df = (header_df.crossJoin(df)
